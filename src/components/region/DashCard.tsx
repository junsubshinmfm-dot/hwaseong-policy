      ...
      >
        ...
        <div className="relative flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-navy font-extrabold text-base leading-tight">{title}</h3>
          ...
          {items.map((item, i) => (
            <div key={item.label} className="relative">
              {i < items.length - 1 && i % 2 === 0 && (
                <div className="absolute right-0 top-1 bottom-1 w-px bg-navy/[0.06]" />
              )}
              <p className="text-navy/30 text-[11px] font-bold uppercase tracking-widest mb-1">
                {item.label}
              </p>
              <p className="text-navy text-xl font-black leading-none">
                {item.value}
              </p>
            </div>
          ))}
